...
## 恢复自定义工具架和脚本
1. 重装后还原工具架 将备份的 .mel 文件复制到新安装 Maya 的对应 prefs\shelves\ 目录中
2. 还原脚本 将脚本文件复制回 scripts 目录
3. 再还原其他必要目录脚本
4. 启动 Maya 后，工具架应会自动加载。

## 推荐的备份方式
- 压缩整个 Maya 配置目录 定期将 C:\Users\<你的用户名>\Documents\maya\ 整个目录压缩备份，以便全面恢复自定义设置和配置。
- 使用版本控制 如果有大量脚本，建议使用 Git 或其他版本控制工具来管理脚本的更新和备份。
