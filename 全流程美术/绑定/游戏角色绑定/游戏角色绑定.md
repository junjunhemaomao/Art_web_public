...
<img src="./The_Last_of_Us.png" width="900" alt="The Last of Us">  

视频参考：   
[公开教程](https://www.bilibili.com/video/BV1qb4y1Y7Nf/?spm_id_from=333.337.search-card.all.click&vd_source=864bdeb1d103a188a8026e9260262525)   
[GDC分享](https://www.youtube.com/watch?v=myZcUvU8YWc)
***

## 前文说明
本文是解析《神秘海域》前技术总监的角色面部绑定：

- 可以对标AAA游戏的面部绑定。
- 经过多次迭代，形成了程序化脚本，大大加速和简化了绑定操作。

有很多原因，列举几点：

1. 完全骨骼绑定。在《神秘海域》，《最后生还者》中运用。
2. 详解原作者不愿公开的 **SDK工具** 核心知识点
3. 原教程是纯英文，对中文学习者不友好。

## 难点详解

#### 创建骨骼
<img src="./facejoint_01.jpg" width="900" alt="facejoint_01.jpg">    
...
<img src="./eye_center.png" width="900" alt="eye_center.png">    

## 脚本功能说明   
- Create Facial Joints：  
创建一个默认的面部骨骼（骨骼是独立的）。集成了一个基本的面部骨骼坐标数据（用一个字典结构来存储）。可以根据具体情况再进行坐标调整。   
- Mirror Selected Joints：  
镜像骨骼。遍历循环对散的骨骼进行镜像操作。如果是Maya默认操作必须要有父子结构，这里利用python的数组容器轻松做到这一点。    
- Create LOC on Joints：  
在每个骨骼上，遍历执行虚拟体创建命令，主要获得一个坐标可以归零的控制点。批量循环的命令，省下了大量重复工作。    
- Parenting Locators：  
